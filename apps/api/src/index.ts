...
function validateArm(value: unknown): Arm { if(value!=='raw'&&value!=='nazare')throw new Error('arm must be raw or nazare'); return value; }

await ensureSchema();
createServer(async (req,res)=>{
  try {
    const url=new URL(req.url??'/',`http://${req.headers.host??'localhost'}`);
    if(url.pathname==='/health'){json(res,200,{ok:true,service:'nazare-wind-tunnel-api',version:6,agentConfigSource:'experiment'});return;}
    if(!authorized(req)){json(res,TOKEN?401:503,{error:TOKEN?'unauthorized':'WIND_TUNNEL_TOKEN is not configured'});return;}

    if(req.method==='POST'&&url.pathname==='/runs'){
      const body=await readBody(req);
      const repository=String(body.repository??''); const sourceSha=String(body.sourceSha??''); const experiment=String(body.experiment??''); const arm=validateArm(body.arm);
      ...
      const spec:RunSpec={runId:randomUUID(),subject:{repository,githubSha:sourceSha},experiment:{path:experiment},arm,agent:{provider:null,model:null,thinking:null,timeoutMs:0},createdAt:now};
      const run=await createRun(spec);
      json(res,202,{runId:run.runId,status:run.status,repository,sourceSha,experiment,arm,agentConfigSource:'experiment'});return;
    }
