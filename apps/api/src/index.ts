...
createServer(async (req, res) => {
  try {
    ...
    if (url.pathname === '/health') {
      json(res, 200, {ok:true, service:'nazare-wind-tunnel-api', version:5, maxModelTimeoutMs:MAX_MODEL_TIMEOUT_MS});
      return;
    }
    ...
    if (req.method === 'GET' && url.pathname === '/status') {
      ...
      const counts = Object.fromEntries(RUN_STATES.map(state => [state, runs.filter(run => run.status === state).length]));
      const active = runs.filter(run => !TERMINAL.has(run.status));
      const workers = active.filter(run => run.workerId).map(run => ({
        runId: run.runId,
        workerId: run.workerId,
        status: run.status,
        leaseUntil: run.leaseUntil,
        healthy: Boolean(run.leaseUntil && Date.parse(run.leaseUntil) > Date.now()),
      }));
      json(res, 200, {counts,activeRuns:active.length,queued:counts.queued ?? 0,workers,at:new Date().toISOString()});
      return;
    }
