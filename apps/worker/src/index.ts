...
async function compileNazare(cwd:string,definition:ExperimentDefinition){
  ...
  const result=await runSubjectProcess('pnpm',['run','nazare:registry','compile',definition.nazare.capabilityId,definition.nazare.requestedChange],cwd,60_000);
...
async function executeRun(claimed:RunState){
  let state=claimed;
  const heartbeat=setInterval(()=>{renewLease(state.runId,WORKER_ID).catch(error=>console.error('lease heartbeat failed',error));},HEARTBEAT_MS);heartbeat.unref();
  ...
  try{
    ...
    await appendEvent({runId:state.runId,type:'experiment.resolved',at:new Date().toISOString(),data:{id:definition.id,agent:resolvedAgent,verificationChecks:definition.verification.length}});
    ...
    await appendEvent({runId:state.runId,type:'agent.started',at:new Date().toISOString(),data:{timeoutMs:agentTimeoutMs,startupTimeoutMs:AGENT_STARTUP_TIMEOUT_MS,idleTimeoutMs:AGENT_IDLE_TIMEOUT_MS,provider:resolvedAgent.provider,model:resolvedAgent.model,thinking:resolvedAgent.thinking,preflight:true}});

    const controller=new AbortController();
    const cancellationWatcher=setInterval(()=>{isCancellationRequested(state.runId).then(cancelled=>{if(cancelled)controller.abort();}).catch(error=>console.error('cancel poll failed',error));},CANCEL_POLL_MS);cancellationWatcher.unref();
    let eventWrites=Promise.resolve();
    const enqueueEvent=(type:string,data?:Record<string,unknown>)=>{eventWrites=eventWrites.then(()=>appendEvent({runId:state.runId,type,at:new Date().toISOString(),data})).then(()=>undefined).catch(error=>console.error('agent event write failed',error));};
    const enqueuePiLine=(line:string)=>{for(const event of normalizePiJsonLine(line))enqueueEvent(event.type,event.data);};
    const agent=await runPi({cwd,prompt,provider:resolvedAgent.provider,model:resolvedAgent.model,thinking:resolvedAgent.thinking??undefined,timeoutMs:agentTimeoutMs,startupTimeoutMs:AGENT_STARTUP_TIMEOUT_MS,idleTimeoutMs:AGENT_IDLE_TIMEOUT_MS,signal:controller.signal,onStdoutLine:enqueuePiLine,onStderrLine:line=>enqueueEvent('agent.stderr',{text:line.slice(0,8_000)})});
    clearInterval(cancellationWatcher);await eventWrites;
    await Promise.all([putArtifact({runId:state.runId,type:'pi.transcript',name:'pi.jsonl',mediaType:'application/x-ndjson',content:agent.stdout}),putArtifact({runId:state.runId,type:'pi.stderr',name:'pi.stderr.log',mediaType:'text/plain',content:agent.stderr})]);
    if(agent.aborted)throw new RunCancelledError();
    if(agent.timedOut){await appendEvent({runId:state.runId,type:'agent.timeout',at:new Date().toISOString(),data:{timeoutMs:agentTimeoutMs,startupTimeoutMs:AGENT_STARTUP_TIMEOUT_MS,idleTimeoutMs:AGENT_IDLE_TIMEOUT_MS,timeoutReason:agent.timeoutReason,firstOutputMs:agent.firstOutputMs,lastActivityAt:agent.lastActivityAt,stdoutBytes:Buffer.byteLength(agent.stdout),stderrBytes:Buffer.byteLength(agent.stderr)}});throw new AgentTimeoutError(agent.timeoutReason,agentTimeoutMs);}
    ...
    if(cancelled){state=await saveRun({...state,status:'cancelled',outcome:null,error:null,errorCode:null,finishedAt});await appendEvent({runId:state.runId,type:'run.cancelled',at:finishedAt,data:{workerId:WORKER_ID}}).catch(()=>{});}
    else{const message=error instanceof Error?error.stack??error.message:String(error);const errorCode=error instanceof AgentTimeoutError?'agent_timeout':error instanceof PreflightError?'preflight_failed':error instanceof ExperimentConfigError?'experiment_invalid':'run_error';state=await saveRun({...state,status:'failed',outcome:null,error:message,errorCode,finishedAt});await putArtifact({runId:state.runId,type:'run.error',name:'error.txt',mediaType:'text/plain',content:message}).catch(()=>{});await appendEvent({runId:state.runId,type:'run.failed',at:finishedAt,data:{error:message,errorCode}}).catch(()=>{});}
  }finally{clearInterval(heartbeat);await cleanSubject(repository).catch(error=>console.error('subject cleanup failed',error));}
}

await ensureSchema();console.log(`Nazare Wind Tunnel worker ${WORKER_ID} ready`);
