...
export const POST: APIRoute = async ({request, cookies}) => {
  ...
  if (!equal(supplied, expected)) return new Response('Unauthorized', {status: 401});
  cookies.set(COOKIE, sessionValue(), {httpOnly: true, secure: true, sameSite: 'strict', path: '/', maxAge: 60 * 60 * 24 * 30});
