...
export async function runVerificationPlan(
  ...
  const results: Array<VerificationCheck & { passed: boolean; exitCode: number; stdout: string; stderr: string; durationMs: number }> = [];
  const tiers = ["structural", "focused", "behavioral", "full"] as const;

  for (const tier of tiers) {
    const tierChecks = checks.filter((check) => (check.tier ?? "behavioral") === tier);
